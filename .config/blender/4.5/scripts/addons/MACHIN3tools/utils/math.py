...
def create_rotation_matrix_from_vector(vector, mx=None):
    normal = mx.to_3x3() @ vector.normalized() if mx else vector.normalized()
...
def compare_quat(q1, q2, precision=8, debug=False):
    ...
    is_equal = bool(round(dot, precision)) and bool(round(q1.angle, precision) == round(q2.angle, precision))
...
def create_coords_bbox(coords, use_numpy=False):
    if use_numpy:
        array = np.array([co for co in coords])
        ...
        xmin = min(coords, key=lambda x: x[0]).x
        xmax = max(coords, key=lambda x: x[0]).x

        ymin = min(coords, key=lambda x: x[1]).y
        ymax = max(coords, key=lambda x: x[1]).y

        zmin = min(coords, key=lambda x: x[2]).z
        zmax = max(coords, key=lambda x: x[2]).z

    bbox = [Vector((xmin, ymin, zmin)), Vector((xmax, ymin, zmin)),
            Vector((xmax, ymax, zmin)), Vector((xmin, ymax, zmin)),
            Vector((xmin, ymin, zmax)), Vector((xmax, ymin, zmax)),
            Vector((xmax, ymax, zmax)), Vector((xmin, ymax, zmax))]
