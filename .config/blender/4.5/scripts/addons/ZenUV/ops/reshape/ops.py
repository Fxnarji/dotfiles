...
from ZenUV.utils.generic import (
    ...
    is_selection_exist_in_object_in_edit_mode,
    get_mesh_data,
    verify_uv_layer
)
...
def do_relax_linked(self, context):

    init_select_mode = context.tool_settings.mesh_select_mode[:]
    objs_names = [obj.name for obj in resort_by_type_mesh_in_edit_mode_and_sel(context)]
    ...
    for obj_name in objs_names:
        ...
        if not is_selection_exist_in_object_in_edit_mode(context, bm, obj):
            bpy.ops.object.mode_set(mode='OBJECT')
            obj.select_set(state=False)
            continue
