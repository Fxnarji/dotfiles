...
ZEN_LIB_NAME = {
    "Linux": "zen_uv_lib_linux_64_v2_0_0.so",
    "Darwin": "zen_uv_lib_mac_intel_64_v2_0_0.dylib",
    "DarwinSilicon": "zen_uv_lib_mac_silicon_64_v2_0_0.dylib",
    "Windows": "zen_uv_lib_win_64_v2_0_0.dll"
    }
    ...
    if LIB is None:
        ...
        if LIB:
            try:
                ...
                Log.error('REGISTER CORE:', e)
                try:
                    unregister_library()
                except Exception as e:
                    Log.error('EMERGENCY UNREGISTER CORE:', e)
    else:
    ...
    def setTaskbarProgress(self, position, max, force=False, text=''):
        if self.lib:
            ...
            if (force or (delta_send.total_seconds() > 0.5)):
                res = self.lib.ZenUvOS_SetTaskbarProgress(self.obj, position, max, text)
                self.tm_progress = datetime.datetime.now()
                return res
        return True
